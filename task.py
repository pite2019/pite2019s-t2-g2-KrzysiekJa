...
class Bank:
    def __init__(self, bank_name = 'Somewhere in Switzerland', bank_money = 10000000):
        ...
        self.__clints_file = “%sfile.txt” % (self.bank_name)
        file = open(self.clints_file, 'x')
        file.close()
    
    def creAccount(self, name, last_name, amount):
        self.bank_money += amount
        file   = open(self.__clints_file, 'a')
        client = {
          'name': name,
          'last_name': last_name,
          'money': amount,
    ...
    def delAccount(self, name, last_name):
        with open(self.__clints_file, 'r') as file:
            lines = file.readlines()
        file = open(self.__clints_file, 'w')
        for indx in lines:
    ...
    def moneyWithdrawal(self, name, last_name, change):
        ...
        with open(self.__clints_file, 'r') as file:
            lines = file.readlines()
        with open(self.__clints_file, 'w') as file:
            for indx in lines:
                if (re.search("^{"name": "%s", "last_name": "%s"" % (name, last_name),indx) != None):
                    tmp  = json.loads(indx)
                    tmp['money'] -= change
...
def main():
    ...
    for i in range (0, len(new_clients_list) - 2, 3):
        ...
        print('Added to ', bank2.bankName, ': ', result)
    
    result = bank3.creAccount('Filipiak', 'Janusz', 45000000)
    print('Added to ', bank3.bankName, ': ', result)
